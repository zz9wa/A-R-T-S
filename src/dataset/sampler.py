...
class ParallelSampler():
    ...
    def worker(self, done_queue, sampled_classes, source_classes):
        ...
        while True:
            ...
            if self.args.embedding_toD =='idf':
...
def task_sampler(data, args):
    ...
    sampled_classes = temp[:args.way-1]
    #sampled_classes=np.zeros(args.way)
    i=0
    count=0
    while count <args.way-1:
        if temp[i]!=0:
            sampled_classes[count]=temp[i]
            count=count+1
        i=i+1
    #print("sampled_classes:", sampled_classes)
    sampled_classes=np.append(sampled_classes,0)
    #print("sampled_classes:", sampled_classes)
