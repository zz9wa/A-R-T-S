...
def test_one(task, model, args):
    ...
    '''if args.embedding != 'mlada' and args.embedding != 'mlada_cnn' :
    ...
    if query_data.shape[1] < 50:
        ...
        # Apply the classifier
    acc, d_acc, loss, x_hat, precious, recall = model['clf'](XS, YS, XQ, YQ, XQ_logitsD, XSource_logitsD, YQ_d, YSource_d, query_data)

    all_sentence_ebd = XQ
    all_avg_sentence_ebd = XQ_avg
    all_label = YQ
        # print(all_sentence_ebd.shape, all_avg_sentence_ebd.shape, all_label.shape)
    #print("test_one_acc:",acc)

    return acc, d_acc, all_sentence_ebd.cpu().detach().numpy(), all_avg_sentence_ebd.cpu().detach().numpy(), all_label.cpu().detach().numpy(), XQ_inputD.cpu().detach().numpy(), query_data.cpu().detach().numpy(), x_hat.cpu().detach().numpy(),precious, recall
...
def test(test_data, model, args, num_episodes, verbose=True, sampled_tasks=None):
    ...
    for task in sampled_tasks:
            ...
            acc1, d_acc1, sentence_ebd, avg_sentence_ebd, sentence_label, word_weight, query_data, x_hat, p, r = test_one(task, model, args)
            if count < 20:
    ...
    return np.mean(acc), np.std(acc), np.mean(precious),np.mean(recall),all_drawn_data
